//
//  VendoredNetflixImageLoaderViewModel.swift
//  Notflix
//
...
extension VendoredNetflixImageCache {
    private static var imageCache = VendoredNetflixImageCache()
    static func getImageCache() -> VendoredNetflixImageCache {
        return imageCache
    }
}

class VendoredNetflixImageLoaderViewModel: ObservableObject {
    @Published var image: UIImage?

    private var cache = VendoredNetflixImageCache.getImageCache()
    ...
    func load() {
        ...
        cancellable = URLSession.shared.dataTaskPublisher(for: url)
            .retry(10)
            .subscribe(on: VendoredNetflixImageLoaderViewModel.imageProcessingQueue)

...
struct AudioProgressView: View {
    ...
    var body: some View {
        VStack(spacing: 5) {
            CustomSlider(value: $playerManager.currentTime, range: 0...playerManager.totalDuration, thumbSize: 15, trackHeight: 3)
                .animation(.linear(duration: 0.1), value: playerManager.currentTime)
                .onAppear {
                    print("AudioProgressView appeared with totalDuration: \(playerManager.totalDuration) and currentTime: \(playerManager.currentTime)")
                }

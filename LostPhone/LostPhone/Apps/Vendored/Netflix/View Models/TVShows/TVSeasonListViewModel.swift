...
class VendoredNetflixTVSeasonListViewModel: ObservableObject {
    @Published var episodes = [VendoredNetflixEpisode]()
    ...
    func load(tvShowId: Int, tvSeasonNumber: Int) {
        isLoading = true
        APIClient().send(VendoredNetflixAPIEndpoints.tvSeason(tvShowId: tvShowId, tvSeasonNumber: tvSeasonNumber))

...
struct LpspAwesomeDropboxView: View {
    var body: some View {
        TabView {
            AwesomeDropboxTabScreen(title: "Offline", icon: "arrow.down.circle", appName: "Dropbox")
                .tabItem { Label("Offline", systemImage: "arrow.down.circle") }
            AwesomeDropboxTabScreen(title: "Account", icon: "person.crop.circle", appName: "Dropbox")
                .tabItem { Label("Account", systemImage: "person.crop.circle") }
            AwesomeDropboxTabScreen(title: "Home", icon: "square.grid.2x2.fill", appName: "Dropbox")
                .tabItem { Label("Home", systemImage: "square.grid.2x2.fill") }
            AwesomeDropboxTabScreen(title: "Files", icon: "square.grid.2x2.fill", appName: "Dropbox")
                .tabItem { Label("Files", systemImage: "square.grid.2x2.fill") }
            AwesomeDropboxTabScreen(title: "Photos", icon: "square.grid.2x2.fill", appName: "Dropbox")
                .tabItem { Label("Photos", systemImage: "square.grid.2x2.fill") }
        }
        .tint(DropboxTokens.dbxCanvas)
    }
}
...
private struct AwesomeDropboxTabScreen: View {
    let title: String
    let icon: String
    let appName: String
    ...
    var body: some View {
        NavigationStack {
            ...
            .background(DropboxTokens.dbxCanvas.ignoresSafeArea())
            .navigationTitle(title)
            .toolbarBackground(DropboxTokens.dbxCanvas, for: .navigationBar)
        }
    }
    ...
    private var header: some View {
        HStack {
            Image(systemName: icon)
                ...
                .foregroundStyle(DropboxTokens.dbxCanvas)
            Text(appName)
                .font(.title2.bold())
            Spacer()
        }
    ...
    private var sampleContent: some View {
        ForEach(0..<6, id: \.self) { i in
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(DropboxTokens.dbxCanvas.opacity(0.12))
                .frame(height: 72)

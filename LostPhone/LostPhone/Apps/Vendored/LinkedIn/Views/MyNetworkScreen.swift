//
//  VendoredLinkedInMyNetworkScreen.swift
//  linkedin_clone
//
...
struct VendoredLinkedInMyNetworkScreen: View {
    var networkData: [VendoredLinkedInNetworkModel]=[
    ...
    var body: some View {
        VStack(){
            ...
            VendoredLinkedInHorizontalDivider(color: .gray, height: 10).opacity(0.3).ignoresSafeArea(.all)
            ...
            ScrollView(.vertical){
                ...
                ForEach((networkData), id: \.id){ data in
                      VendoredLinkedInInvitationView(profileData: data)
                    Divider()
                   }
            }
        }

//
//  VendoredAirbnbLoginModel.swift
//  AirbnbClone-SwiftUI
//
...
struct VendoredAirbnbLoginModel: Codable {

        let body: VendoredAirbnbBody?
        let hTTPSTATUSCODE: Int?
        let message: String?
        let status: Bool?

        enum VendoredAirbnbCodingKeys: String, CodingKey {
                case body = "body"
        ...
        init(from decoder: Decoder) throws {
                let values = try decoder.container(keyedBy: VendoredAirbnbCodingKeys.self)
                body = try values.decodeIfPresent(VendoredAirbnbBody.self, forKey: .body)
...
struct VendoredAirbnbBody: Codable {

...
struct VendoredNetflixTVSeasonListView: View {
    @ObservedObject private var tvSeasonListViewModel: VendoredNetflixTVSeasonListViewModel

    let tvSeasonName: String
    init(tvShowId: Int, tvSeason: VendoredNetflixTVSeason) {
        self.tvSeasonName = tvSeason.name
        self.tvSeasonListViewModel = VendoredNetflixTVSeasonListViewModel(tvShowId: tvShowId, tvSeasonNumber: tvSeason.seasonNumber)
    }
    ...
    var body: some View {
        Group {
            if !self.tvSeasonListViewModel.episodes.isEmpty {
                    ...
                    ForEach (self.tvSeasonListViewModel.episodes.sorted { $0.episodeNumber > $1.episodeNumber }, id: \.id) { episode in
                        HStack(alignment: .top, spacing: 10) {
                            ZStack(alignment: .center) {
                                Group {
                                    if episode.stillUrl != nil {
                                        VendoredNetflixAsyncImage(url: episode.stillUrl!,
                            ...
                            VStack(alignment: .leading) {
                                ...
                                Text("VendoredNetflixEpisode \(episode.episodeNumber)").font(.system(size: 14, weight: .medium))
                ...
                Group {
                    VendoredNetflixShimmerView().frame(height: 40)
                        .padding(.top, 30)
                    ForEach((1...5), id: \.self) { _ in
                        VendoredNetflixShimmerView().frame(height: 100)
                    }
                }
            }
        }

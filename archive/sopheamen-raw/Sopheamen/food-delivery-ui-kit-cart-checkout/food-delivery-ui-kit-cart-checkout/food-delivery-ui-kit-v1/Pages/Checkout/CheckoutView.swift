...
#Preview {
    CheckoutView(cart: cartData[0])
}

struct MapCartView:View {
    // map const
...
struct AddressCartView:View {
    var addressDatas:[AddressResponse] = addressData
    var body: some View {
        LazyVStack {
            ForEach(addressDatas) { address in
                AddressCartRowView(address: address)
            }
        }
    }
}

struct AddressCartRowView:View {
...
struct DeliveryOptionView:View {
    var deliveryOptionsDatas:[DeliveryOptionResponse] = deliveryOptionsData
        ...
        VStack (spacing:12){
            ...
            VStack (spacing:12){
                ForEach(deliveryOptionsDatas) { delivery in
                    DeliveryOptionRowView(delivery: delivery, selectedId: $selectedId)
                }
            }
        }
    }
...
struct DeliveryOptionRowView: View {
    var delivery: DeliveryOptionResponse

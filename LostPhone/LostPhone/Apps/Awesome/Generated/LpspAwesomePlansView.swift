...
struct LpspAwesomePlansView: View {
    var body: some View {
        TabView {
            AwesomePlansTabScreen(title: "Contribute", icon: "plus.circle.fill", appName: "Plans")
                .tabItem { Label("Contribute", systemImage: "plus.circle.fill") }
            AwesomePlansTabScreen(title: "Explore", icon: "square.grid.2x2.fill", appName: "Plans")
                .tabItem { Label("Explore", systemImage: "square.grid.2x2.fill") }
            AwesomePlansTabScreen(title: "Go", icon: "square.grid.2x2.fill", appName: "Plans")
                .tabItem { Label("Go", systemImage: "square.grid.2x2.fill") }
            AwesomePlansTabScreen(title: "Saved", icon: "square.grid.2x2.fill", appName: "Plans")
                .tabItem { Label("Saved", systemImage: "square.grid.2x2.fill") }
            AwesomePlansTabScreen(title: "Updates", icon: "square.grid.2x2.fill", appName: "Plans")
                .tabItem { Label("Updates", systemImage: "square.grid.2x2.fill") }
        }
        .tint(PlansTokens.gmCanvas)
    }
}
...
private struct AwesomePlansTabScreen: View {
    let title: String
    let icon: String
    let appName: String
    ...
    var body: some View {
        NavigationStack {
            ...
            .background(PlansTokens.gmCanvas.ignoresSafeArea())
            .navigationTitle(title)
            .toolbarBackground(PlansTokens.gmCanvas, for: .navigationBar)
        }
    }
    ...
    private var header: some View {
        HStack {
            Image(systemName: icon)
                ...
                .foregroundStyle(PlansTokens.gmCanvas)
            Text(appName)
                .font(.title2.bold())
            Spacer()
        }
    ...
    private var sampleContent: some View {
        ForEach(0..<6, id: \.self) { i in
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(PlansTokens.gmCanvas.opacity(0.12))
                .frame(height: 72)

...
struct LpspAwesomeBanqueView: View {
    var body: some View {
        TabView {
            AwesomeBanqueTabScreen(title: "Home", icon: "house.fill", appName: "Banque")
                .tabItem { Label("Home", systemImage: "house.fill") }
            AwesomeBanqueTabScreen(title: "Invest", icon: "chart.line.uptrend.xyaxis", appName: "Banque")
                .tabItem { Label("Invest", systemImage: "chart.line.uptrend.xyaxis") }
            AwesomeBanqueTabScreen(title: "Crypto", icon: "bitcoinsign.circle.fill", appName: "Banque")
                .tabItem { Label("Crypto", systemImage: "bitcoinsign.circle.fill") }
            AwesomeBanqueTabScreen(title: "Lifestyle", icon: "sparkles", appName: "Banque")
                .tabItem { Label("Lifestyle", systemImage: "sparkles") }
            AwesomeBanqueTabScreen(title: "Cards", icon: "creditcard.fill", appName: "Banque")
                .tabItem { Label("Cards", systemImage: "creditcard.fill") }
        }
        .tint(BanqueTokens.revCanvas)
    }
}

private enum BanqueTokens {
...
private struct AwesomeBanqueTabScreen: View {
    let title: String
    let icon: String
    let appName: String
    ...
    var body: some View {
        NavigationStack {
            ...
            .background(BanqueTokens.revCanvas.ignoresSafeArea())
            .navigationTitle(title)
            .toolbarBackground(BanqueTokens.revCanvas, for: .navigationBar)
        }
    }
    ...
    private var header: some View {
        HStack {
            Image(systemName: icon)
                ...
                .foregroundStyle(BanqueTokens.revCanvas)
            Text(appName)
                .font(.title2.bold())
            Spacer()
        }
    ...
    private var sampleContent: some View {
        ForEach(0..<6, id: \.self) { i in
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(BanqueTokens.revCanvas.opacity(0.12))
                .frame(height: 72)

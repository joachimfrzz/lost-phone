...
public class APIClient {
    ...
    func send<X>(_ request: VendoredNetflixAPIRequest<X>) -> AnyPublisher<X, Error> {
    ...
    private func validate(_ data: Data, _ response: URLResponse) throws -> Data {
        guard let httpResponse = response as? HTTPURLResponse else {
            throw VendoredNetflixAPIError.invalidResponse
        }
        guard (200..<300).contains(httpResponse.statusCode) else {
            throw VendoredNetflixAPIError.statusCode
        }
        return data
    }

    private func endpoint<X>(for request: VendoredNetflixAPIRequest<X>) -> URL? {

//
//  VendoredNetflixHomeMoviesViewModel.swift
//  Notflix
//
...
class VendoredNetflixHomeMoviesViewModel: ObservableObject {
    ...
    private func fetchGenres() {
        self.state = .loading
        APIClient().send(VendoredNetflixAPIEndpoints.movieGenres).sink(receiveCompletion: { (completion) in
            switch completion {
            case .failure:
                self.state = .error
                self.genres = []

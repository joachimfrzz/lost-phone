//
//  VendoredPayPalContentView.swift
//  PayPal_clone
//
...
struct VendoredPayPalContentView: View {
    
    @State private var showReferral = true
    @State var scrollProxy: ScrollViewProxy?
    @StateObject var headerData = VendoredPayPalHeaderViewModel()
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                Color(vendoredPayPalHex: "#E7EFF6").ignoresSafeArea(.all)
                VStack(spacing: 0) {
                    VendoredPayPalTopMenu()
                      // .offset(y: headerData.headerOffset)
                    VendoredPayPalBackToTopScrollView(scrollProxy: $scrollProxy) { proxy in
                        ScrollView(.vertical, showsIndicators: false) {
                            VStack {
                                HStack(spacing: 20){
                                    VendoredPayPalBalanceTile()
                                    VendoredPayPalInfoTile()
                                }
                                .padding()
                                VendoredPayPalPayLaterTile()
                                VendoredPayPalProfilePicturesScrollable()
                                if showReferral {
                                    VendoredPayPalReferralInfo(showReferral: $showReferral)
                                }
                                VendoredPayPalSendMoneyInternationally()
...
//                                        print("startMinY: \(headerData.startMinY), minY: \(minY), offset: \(offset), headerOffset: \(headerData.headerOffset)")
                ...
                VendoredPayPalBottomNav(scrollProxy: $scrollProxy)
                    .ignoresSafeArea(.all)
            }
        }
    }

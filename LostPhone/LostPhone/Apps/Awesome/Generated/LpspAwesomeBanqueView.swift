...
struct LpspAwesomeBanqueView: View {
    var body: some View {
        TabView {
            BanqueTabHost(title: "Home", icon: "house.fill") { BanqueScreenFactory.screen(for: "Home") }
                .tabItem { Label("Home", systemImage: "house.fill") }
            BanqueTabHost(title: "Invest", icon: "chart.line.uptrend.xyaxis") { BanqueScreenFactory.screen(for: "Invest") }
                .tabItem { Label("Invest", systemImage: "chart.line.uptrend.xyaxis") }
            BanqueTabHost(title: "Crypto", icon: "bitcoinsign.circle.fill") { BanqueScreenFactory.screen(for: "Crypto") }
                .tabItem { Label("Crypto", systemImage: "bitcoinsign.circle.fill") }
            BanqueTabHost(title: "Lifestyle", icon: "sparkles") { BanqueScreenFactory.screen(for: "Lifestyle") }
                .tabItem { Label("Lifestyle", systemImage: "sparkles") }
            BanqueTabHost(title: "Cards", icon: "creditcard.fill") { BanqueScreenFactory.screen(for: "Cards") }
                .tabItem { Label("Cards", systemImage: "creditcard.fill") }
        }
        .tint(BanqueTokens.revBrand)
    }
}

private struct BanqueTabHost<Content: View>: View {
    let title: String
    let icon: String
...
private enum BanqueScreenFactory {
    @ViewBuilder
    static func screen(for tab: String) -> some View {
        ...
        case let t where t.contains("recipient") || t.contains("plus"):
            BanqueMoreScreen()
        default:
            BanqueGenericListScreen(title: tab)
        }
    }
}
...
private struct BanqueHomeScreen: View {
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(spacing: 20) {
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Solde total").font(.system(size: 14)).foregroundStyle(BanqueTokens.revTextSecondary)
                        Text("2 847,50 €").font(.system(size: 36, weight: .bold)).foregroundStyle(BanqueTokens.revTextPrimary)
                        HStack(spacing: 16) {
                            Label("Ajouter", systemImage: "plus")
                            Label("Envoyer", systemImage: "arrow.up.right")
                            Label("Échanger", systemImage: "arrow.left.arrow.right")
                        }.font(.system(size: 14, weight: .semibold)).foregroundStyle(BanqueTokens.revBrand).padding(.top, 4)
                    }.frame(maxWidth: .infinity, alignment: .leading)
                    RoundedRectangle(cornerRadius: 16).fill(LinearGradient(colors: [BanqueTokens.revBrand, BanqueTokens.revBrand.opacity(0.6)], startPoint: .topLeading, endPoint: .bottomTrailing))
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        ForEach(BanqueDemoData.transactions) { tx in BanqueTransactionRow(tx: tx) }
                    }
                }.padding()
            }.background(BanqueTokens.revCanvas.ignoresSafeArea()).navigationTitle("Banque")
        }
    }
}

private struct BanqueTransactionRow: View {
    let tx: BanqueTransaction
    var body: some View {
        HStack(spacing: 12) {
...
private struct BanqueCardsScreen: View {
    var body: some View { NavigationStack { Text("Gérez vos cartes").padding().navigationTitle("Cartes") } }
}

private struct BanqueMoreScreen: View {
    var body: some View {
        NavigationStack { List { Section("Compte") { Label("Profil", systemImage: "person"); Label("Sécurité", systemImage: "lock") } }.navigationTitle("Plus") }
    }
}

private struct BanqueTransaction: Identifiable { let id = UUID(); let title: String; let date: String; let amount: String; let icon: String; let color: Color }
private enum BanqueDemoData {
    static let transactions: [BanqueTransaction] = [
...
private struct BanqueGenericListScreen: View {
    let title: String
    var body: some View {
        NavigationStack {
            List(0..<8, id: \.self) { i in
                HStack {
                    RoundedRectangle(cornerRadius: 8).fill(BanqueTokens.revBrand.opacity(0.15)).frame(width: 44, height: 44)

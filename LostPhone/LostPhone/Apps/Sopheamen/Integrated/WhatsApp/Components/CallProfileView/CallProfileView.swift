...
struct CallProfileView: View {
    ...
    var body: some View {
        VStack (spacing:8){
            HStack (spacing: 10){
                ProfileImageView(profileImage: callResponse.user.profileUrl, size: 45)
                    .overlay(Circle().stroke(.gray.opacity(0.2)))
                VStack (alignment: .leading,spacing: -3){
                    Text(callResponse.user.name)
                        .font(.headline)
                        .fontWeight(.regular)
                        .foregroundStyle(callResponse.type == 1 ? Color.dangerColor : .black)
...
#Preview {
    CallProfileView(callResponse: callData[0])
}

...
struct VendoredPayPalBackToTopScrollView<Content: View>: View {
...
struct VendoredPayPalBottomNav: View {
    @Binding var scrollProxy: ScrollViewProxy?
        ...
        VStack {
            ZStack {
                Rectangle()
                    .fill(Color.white)
                    .frame(height: 120)
                    .shadow(color: Color(vendoredPayPalHex: "#B9BFC5"), radius: 10)
                HStack(spacing: 60) {
                                   Button(action: {
                                       withAnimation {
...
#Preview {
    ...
    return VendoredPayPalBottomNav(scrollProxy: $scrollProxy)
}

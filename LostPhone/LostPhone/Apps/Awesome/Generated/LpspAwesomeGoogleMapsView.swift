...
struct LpspAwesomeGoogleMapsView: View {
    var body: some View {
        TabView {
            GoogleMapsTabHost(title: "Contribute", icon: "plus.circle.fill") { GoogleMapsScreenFactory.screen(for: "Contribute") }
                .tabItem { Label("Contribute", systemImage: "plus.circle.fill") }
        }
        .tint(GoogleMapsTokens.gmTextPrimary)
    }
}

private struct GoogleMapsTabHost<Content: View>: View {
    let title: String
    let icon: String
...
private enum GoogleMapsScreenFactory {
    @ViewBuilder
    static func screen(for tab: String) -> some View {
        ...
        case let t where t.contains("activ") || t.contains("clock"):
            GoogleMapsActivityScreen()
        default:
            GoogleMapsGenericListScreen(title: tab)
        }
    }
}
...
private struct GoogleMapsMapScreen: View {
    var body: some View {
        ZStack(alignment: .bottom) {
            GoogleMapsMapPlaceholder()
            GoogleMapsRideSheet(accent: GoogleMapsTokens.gmTextPrimary, surface: GoogleMapsTokens.gmSurfaceMuted)
        }.ignoresSafeArea(edges: .bottom)
    }
}

private struct GoogleMapsMapPlaceholder: View {
    var body: some View {
        ZStack {
...
private struct GoogleMapsRideSheet: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                ForEach(GoogleMapsDemoData.rides) { ride in
                    HStack {
                ...
                Button("Confirmer Google Maps") {}.frame(maxWidth: .infinity).padding().background(RoundedRectangle(cornerRadius: 10).fill(accent)).foregroundStyle(.white).font(.headline)
            }.padding()
        }.background(RoundedRectangle(cornerRadius: 16).fill(.regularMaterial))
    }
}

private struct GoogleMapsActivityScreen: View {
    var body: some View {
        NavigationStack {
            List(GoogleMapsDemoData.rides) { r in
...
private struct GoogleMapsRide: Identifiable { let id = UUID(); let name: String; let eta: String; let price: String; let icon: String }
private enum GoogleMapsDemoData {
    static let rides: [GoogleMapsRide] = [
...
private struct GoogleMapsGenericListScreen: View {
    let title: String
    var body: some View {
        NavigationStack {
            List(0..<8, id: \.self) { i in
                HStack {
                    RoundedRectangle(cornerRadius: 8).fill(GoogleMapsTokens.gmTextPrimary.opacity(0.15)).frame(width: 44, height: 44)
